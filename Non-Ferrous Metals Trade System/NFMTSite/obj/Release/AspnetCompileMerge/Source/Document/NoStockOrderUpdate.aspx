<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="NoStockOrderUpdate.aspx.cs" Inherits="NFMTSite.Document.NoStockOrderUpdate" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            var orderTypeSource = { datatype: "json", url: "../BasicData/Handler/OrderTypeHandler.ashx?id=2", async: false };
            ...
            selectedStocks = selectedDataAdapter.records;

            $("#jqxOrderStocksGrid").on("cellvaluechanged", function (event) {
            ...
            $("#btnAddStock").jqxButton();

            //申请公司
            var ownCorpUrl = "../User/Handler/AuthSelfCorpHandler.ashx?SubId="+"<%=this.curOrder.SubId%>";
            ...
            //客户公司
            var buyCorpUrl = "../User/Handler/CorpDDLHandler.ashx?isSelf=0&SubId=" + "<%=this.curOrder.SubId%>";
            ...
            //毛重
            $("#txbGrossAmount").jqxNumberInput({ height: 25, spinButtons: true, decimalDigits: 4, Digits: 8 });
            ...
            //捆数
            $("#txbBundles").jqxNumberInput({ height: 25, spinButtons: true, decimalDigits: 0, Digits: 4 });           
            ...
            $("#btnUpdateOrder").jqxInput();

            $("#jqxNeedExpander").jqxValidator({
                rules:
                    [
                        {
                            input: "#selOrderType", message: "请优先选择制单指令类型", action: "select", rule: function (input, commit) {
                                return $("#selOrderType").val() > 0;
                            }
                        },
                        {
                            input: "#txbInvoicePrice", message: "请优先填写发票价格", action: "textchanged", rule: function (input, commit) {
            ...
            $("#jqxValidator").jqxValidator({
                rules:
                    [
                        ...
                        {
                            input: "#selBuyerCorp", message: "请选择客户公司", action: "change", rule: function (input, commit) {
                                return $("#selBuyerCorp").val() > 0;
                            }
                        },
                        {
            ...
            //弹窗
            $("#txbStockRefNo").jqxInput({ height: 25, width: 120 });
            $("#txbStockApplyAmount").jqxNumberInput({ width: 120, height: 25, decimalDigits: 4, spinButtons: true });
            $("#popupWindow").jqxWindow({ width: 250, height: 160, resizable: false, isModal: true, autoOpen: false, cancelButton: $("#btnStockCancel"), modalOpacity: 0.2 });
            $("#btnStockCancel").jqxButton({ height: 25, width: 100 });
            $("#btnStockSave").jqxButton({ height: 25, width: 100 });
            ...
            $("#popupWindow").jqxValidator({
                rules:
                    [
                        { input: "#txbStockRefNo", message: "请输入业务单号", action: "keyup,blur", rule: "required" },
                        {
                            input: "#txbStockApplyAmount", message: "申请重量不能小于0", action: "change", rule: function (input, commit) {
                                return $("#txbStockApplyAmount").jqxNumberInput("val") > 0;
                            }
                        }
                    ]
            });
            ...
            $("#btnStockSave").click(function () {
                ...
                var datarow = {
                    RefNo: $("#txbStockRefNo").val(),
                    ApplyWeight: $("#txbStockApplyAmount").val(),
                    MUName: "<%=this.curUnit.MUName%>",
                    AssetName: "<%=this.curAsset.AssetName%>"
                };

                var commit = $("#jqxOrderStocksGrid").jqxGrid("addrow", null, datarow);
                $("#popupWindow").jqxWindow("hide");
                selectedStocks.push(datarow);
                TotalValue(true);
                FlushGrid();
            });
        ...
        function bntRemoveOnClick(row) {
            ...
            var item = $("#jqxOrderStocksGrid").jqxGrid("getrowdata", row);
        ...
        function UpdateOrder() {
            ...
            var OrderDetail = {
                DetailId: "<%=this.curOrderDetail.DetailId%>",
            ...
            $.post("Handler/OrderUpdateHandler.ashx", { order: JSON.stringify(Order), orderStockInvoice: JSON.stringify(selectedStocks), orderDetail: JSON.stringify(OrderDetail) },
                   function (result) {
                       var obj = JSON.parse(result);
                       if (obj.ResultStatus.toString() == "0") {
                           AjaxFileUpload(fileIds, <%=this.curOrder.OrderId%>, AttachTypeEnum.OrderAttach);
                       }
                       alert(obj.Message);
                       if (obj.ResultStatus.toString() == "0") {
...
<body>
        ...
        <div id="jqxNeedExpander" style="float: left; margin: 0px 5px 5px 5px;">
            ...
            <div class="InfoExpander">
                <ul>
                    ...
                    <li>
                        <input type="button" value="添加库存信息" id="btnAddStock"style="width: 120px; height: 25px;" />
                    </li>
                </ul>
            </div>
        </div>        
    ...
    <div id="buttons" style="width: 80%; text-align: center;">
        <input type="button" value="修改制单指令" id="btnUpdateOrder" onclick="UpdateOrder();" style="width: 120px; height: 25px;" />
    </div>
        ...
        <div class="InfoExpander">
            <ul>
                <li>
                    ...
                    <input type="text" id="txbStockRefNo" style="float: left;" />
                </li>
                <li>
                    <span style="float:left;">申请重量：</span>
                    <div id="txbStockApplyAmount" style="float: left;"></div>
                </li>
                <li>
                    <input style="margin-right: 5px;" type="button" id="btnStockSave" value="确认" />
                    <input id="btnStockCancel" type="button" value="取消" />
                </li>
            </ul>
        </div>
    </div>
