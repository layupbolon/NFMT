<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SubUpdate.aspx.cs" Inherits="NFMTSite.Contract.SubUpdate" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            //贴现费/率
            $("#txbDiscountRate").jqxNumberInput({ height: 25, min: 0, decimalDigits: 4,Digits: 3,symbol: "%", symbolPosition: "right", width: 140, spinButtons: true, disabled: true });
            ...
            $("#selCurrency").val(<%=this.curSub.SettleCurrency%>);
            $("#selUnit").val(<%=this.curSub.UnitId%>);
            $("#txbSignAmount").val(<%=this.curSub.SignAmount%>);            
            $("#txbMemo").val("<%=this.curSub.Memo%>");
            ...
            $("#btnUpdate").jqxButton({ width: 120, height: 25 }); $("#btnCancel").jqxButton({ width: 120, height: 25 });
            ...
            $("#btnUpdate").click(function () {
                ...
                $.post("Handler/SubUpdateHandler.ashx", { Sub: JSON.stringify(sub), SubDetail: JSON.stringify(subDetail), SubPrice: JSON.stringify(subPrice), OutCorps:  JSON.stringify(outCorps), InCorps: JSON.stringify(inCorps) },
                    function (result) {
                        var obj = JSON.parse(result);
                        if (obj.ResultStatus.toString() == "0") {
                            AjaxFileUpload(fileIds, "<%=this.curSub.SubId%>", AttachTypeEnum.SubAttach);
                        }
                        alert(obj.Message);
                        if (obj.ResultStatus.toString() == "0") {
...
<body>
    ...
    <div id="jqxValidator">
        ...
        <NFMT:Attach runat="server" ID="attach1" AttachStyle="Update" AttachType="SubAttach" />

        <div style="width: 80%; text-align: center;">
            <input type="button" id="btnUpdate" value="修改子合约" />
