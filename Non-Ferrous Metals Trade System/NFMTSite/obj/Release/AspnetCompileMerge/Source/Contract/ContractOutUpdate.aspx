<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ContractOutUpdate.aspx.cs" Inherits="NFMTSite.Contract.ContractOutUpdate" %>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("#selTradeDirection").jqxDropDownList({ source: directionDataAdapter, displayMember: "DetailName", valueMember: "StyleDetailId", width: 100, height: 25, autoDropDownHeight: true, selectedIndex: 0, disabled: true, disabled: true });
        ...
        //延期费/率
        var valueRateTypeModeStyle = "<%=(int)NFMT.Data.StyleEnum.ValueRateType%>";;
        ...
        var discountBaseModeStyle = "<%=(int)NFMT.Data.StyleEnum.DiscountBase%>";
        ...
        function ContractUpdate() {
            ...
            $.post("Handler/ContractOutUpdateHandler.ashx", { Contract: JSON.stringify(contract), ContractDetail: JSON.stringify(contractDetail), ContractPrice: JSON.stringify(contractPrice), OutCorps: outCorps, InCorps: inCorps, Depts: depts, sub: JSON.stringify(subContract), checkedClause: JSON.stringify(eval(checkedClause)),ContractTypes: contractTypes },
                function (result) {
                    var obj = JSON.parse(result);
                    if (obj.ResultStatus.toString() == "0") {
                        AjaxFileUpload(fileIds, "<%=this.curContract.ContractId%>", AttachTypeEnum.SubAttach);
                        AjaxFileUpload(fileIds, "<%=this.curSub.SubId%>", AttachTypeEnum.ContractAttach);                        
                    }
                    alert(obj.Message);
                    if (obj.ResultStatus.toString() == "0") {
    ...
    $(document).ready(function () {
        ...
        $("#jqxgrid").jqxGrid(
        {
            ...
            columns: [
              ...
              { text: "关内外", datafield: "CustomsTypeName", editable: false },
              { text: "重量单位", datafield: "MUName", editable: false },
              { 
                  text: "销售净重", datafield: "NetAmount", columntype: "numberinput", cellclassname: "GridFillNumber", sortable: false,
                  validation: function (cell, value) {
                      var item = dataAdapter.records[cell.row];
                      if (value < 0 || value > item.OutAmount) {
                          return { result: false, message: "申请重量不能小于0且大于剩余重量" + item.OutAmount };
                      }
                      return true;
                  },
                  createeditor: function (row, cellvalue, editor) {
                      var item = dataAdapter.records[row];
                      editor.jqxNumberInput({ min: 0, decimalDigits: 4, Digits: 8, spinButtons: true, symbolPosition: "right", symbol: item.MUName });
                  },
                  aggregates: [{ '总':
              ...
              { 
                  text: "销售捆数", datafield: "Bundles", columntype: "numberinput", cellclassname: "GridFillNumber", sortable: false,
                  validation: function (cell, value) {
                      var item = dataAdapter.records[cell.row];
                      if (value <= 0 || value > item.OutBundles) {
                          return { result: false, message: "申请捆数不能小于0且大于剩余捆数" + item.OutBundles };
                      }
                      return true;
                  },
                  createeditor: function (row, cellvalue, editor) {
