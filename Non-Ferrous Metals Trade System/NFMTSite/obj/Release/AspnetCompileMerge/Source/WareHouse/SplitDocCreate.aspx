<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SplitDocCreate.aspx.cs" Inherits="NFMTSite.WareHouse.SplitDocCreate" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            var url = "Handler/SplitDocDetailListHandler.ashx";
            var source =
            {
                localdata: "",
                datafields:
                ...
                columns: [
                  ...
                  { text: "权证编号", datafield: "PaperNo", cellclassname: "GridFillNumber" },
                  {
                      ...
                      createeditor: function (row, cellvalue, editor, cellText, width, height) {
                          editor.jqxComboBox({ source: outCorpDataAdapter, displayMember: "Name", valueMember: "EmpId" });
                      }
                  },
            ...
            var stock = <%=this.stockJson%>;
            $("#txbNewCardNo").val(stock.CardNo);
            $("#txbNewPaperNo").val(stock.PaperNo);
            $("#txbNewMemo").val(stock.Memo);

            $("#btnSplit").click(function () {
                $("#popupWindow").jqxWindow("show");

                $("#txbNewRefNo").val("<%=this.stockName.RefNo%>");
                $("#nbNewGrossAmount").val(stock.CurGrossAmount);
                $("#nbNewNetAmount").val(stock.CurNetAmount);
                $("#nbNewBundles").val(stock.Bundles);
                $("#ddlNewPaperHolder").val(Stock.PaperHolder);
            });
            ...
            $("#Save").click(function () {
                ...
                //set stock model
                stock.CurGrossAmount -= $("#nbNewGrossAmount").val();
                stock.CurNetAmount -= $("#nbNewNetAmount").val();
                stock.Bundles -= $("#nbNewBundles").val();
                ...
                var datarow = {
                    ...
                    AssetName: "<%=NFMT.Data.BasicDataProvider.Assets.FirstOrDefault(a=>a.AssetId==this.stock.AssetId).AssetName%>",
                    BrandId: "<%=this.stock.BrandId%>",
                    Bundles: $("#nbNewBundles").val(),
                    BrandName: "<%=NFMT.Data.BasicDataProvider.Brands.FirstOrDefault(a=>a.BrandId==this.stock.BrandId).BrandName%>",
            ...
            $("#btnSave").on("click", function () {
                ...
                if (splitGrossAmount < "<%=this.stock.GrossAmount%>" || splitNetAmount < "<%=this.stock.NetAmount%>" || splitBundles < "<%=this.stock.Bundles%>") {
                    var leftGrossAmount = parseFloat("<%=this.stock.GrossAmount%>") - parseFloat(splitGrossAmount);
                    var leftNetAmount = parseFloat("<%=this.stock.NetAmount%>") - parseFloat(splitNetAmount);
                    var leftBundles = "<%=this.stock.Bundles%>" - splitBundles;
                    if (!confirm("是否将剩余 【毛重】：" + Math.round(leftGrossAmount * 10000) / 10000 + muName + "，【净重】：" + Math.round(leftNetAmount * 10000) / 10000 + muName + "，【捆数】：" + leftBundles + " 作为一张单据拆分？")) {
                        return;
                    }
                ...
                $.post("Handler/SplitDocCreateHandler.ashx", {
                    ...
                    splitDetail: JSON.stringify(rows)
                     ...
                     function (result) {
                         ...
                         if (obj.ResultStatus.toString() == "0") {
                             AjaxFileUpload(fileIds, obj.ReturnValue, AttachTypeEnum.SplitDocAttach);
                         }
                         alert(obj.Message);
                         if (obj.ResultStatus.toString() == "0") {
...
<body>
    ...
    <NFMT:Attach runat="server" ID="attach1" AttachStyle="Create" AttachType="SplitDocAttach" />

    <div style="text-align: center; width: 80%; margin-top: 0px;">
        <input type="button" id="btnSave" value="保存拆单" runat="server" style="width: 120px; height: 25px;" />&nbsp;&nbsp;&nbsp;
        <a target="_self" runat="server" href="SplitDocStockList.aspx" id="btnCancel" style="margin-left: 10px">取消</a>
    </div>
    <div id="popupWindow">
        <div>拆单</div>
        <div id="layOutDiv">
