<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PreToRealCreate.aspx.cs" Inherits="NFMTSite.WareHouse.PreToRealCreate" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#selStockType").jqxComboBox({ source: stockTypeDataAdapter, displayMember: "DetailName", valueMember: "StyleDetailId", width: 120, height: 25, autoDropDownHeight: true, selectedIndex: 0 });
            ...
            $("#selStockOperateType").jqxComboBox({ source: stockOperateTypeDataAdapter, displayMember: "DetailName", valueMember: "StyleDetailId", width: 120, height: 25, autoDropDownHeight: true, selectedIndex: 0 });
            ...
            $("#jqxExpander").jqxValidator({
                rules:
                    [
                        {
                            input: "#txbCardNo", message: "请输入卡号或提单号", action: "keyup,blur", rule: function (input, commit) {
                                return $("#txbCardNo").jqxInput("val").length > 0 || $("#txbStockNo").jqxInput("val").length > 0;
                            }
                        },
                        {
                            input: "#txbStockNo", message: "请输入提单号或卡号", action: "keyup,blur", rule: function (input, commit) {
                                return $("#txbCardNo").jqxInput("val").length > 0 || $("#txbStockNo").jqxInput("val").length > 0;
                            }
                        },
                        { input: "#txbRefNo", message: "请输入业务单号", action: "keyup,blur", rule: "required" },
                        {
                            input: "#selStockType", message: "必须选择单据类型", action: "change,keyup,blur", rule: function (input, commit) {
                                return $("#selStockType").jqxComboBox("val") > 0;
                            }
                        },
                        {
                            input: "#selStockOperateType", message: "必须选择入库类型", action: "change,keyup,blur", rule: function (input, commit) {
                                return $("#selStockOperateType").jqxComboBox("val") > 0;
                            }
                        }
                    ]
            });            

            function StockInfoUpdate() {
                ...
                $.post("Handler/StockInfoUpdateHandler.ashx", {
                    stockId:"<%=this.stock.StockId%>",
                    cardNo: $("#txbCardNo").val(),
                    stockNo: $("#txbStockNo").val(),
                    refNo: $("#txbRefNo").val(),
                    stockType: $("#selStockType").jqxComboBox("val"),
                    stockOperateType: $("#selStockOperateType").jqxComboBox("val")
                },
                    function (result) {
                        var obj = JSON.parse(result);
                        if (obj.ResultStatus.toString() == "0") {
                            AjaxFileUpload(fileIds, "<%=this.stock.StockId%>", AttachTypeEnum.StockAttach);
                        }
                        if (obj.ResultStatus.toString() == "0") {
                            AutoSubmitAudit(MasterEnum.预入库转正式审核, JSON.stringify(obj.ReturnValue));
                        }
                        alert(obj.Message);                        
                        if (obj.ResultStatus.toString() == "0") {
                            document.location.href = "PreToRealList.aspx";
                        }
                    }
                );
            }

            $("#btnAdd").click(function () { StockInfoUpdate(); });
        });
    </script>
...
<body>
    ...
    <NFMT:Attach runat="server" ID="attach1" AttachStyle="Create" AttachType="StockAttach" />
    ...
    <div id="jqxExpander" style="float: left; margin: 0px 5px 5px 5px;">
        ...
        <div id="layOutDiv">
            <ul>
                ...
                <%--<li>
                    <span style="width: 15%; text-align: right;">单据附件：</span>
                    <input type="file" id="fileStockAttach" name="fileStockAttach" class="StockAttach"/>
                </li>--%>
                <li>
                    <span style="text-align: right; width: 15%;">&nbsp;</span>
                    <span>
                        <input type="button" id="btnAdd" value="确认并提交审核" runat="server" style="margin-left: 65px" /></span>
                    <span><a target="_self" runat="server" href="PreToRealList.aspx" id="btnCancel" style="margin-left: 10px">取消</a></span>
                </li>
            </ul>
        </div>
    </div>
