<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="FinancingRepoApplyUpdate.aspx.cs" Inherits="NFMTSite.Financing.FinancingRepoApplyUpdate" %>
...
<body>
        ...
        $(document).ready(function () {
            ...
            $("#jqxStockGrid").jqxGrid(
            {
                ...
                columns: [
                  ...
                  //},
                  //{
                  //    text: "赎回净重", datafield: "NetAmount", sortable: false, width: "10%", columntype: "numberinput", cellclassname: "GridFillNumber",
                  //    validation: function (cell, value) {
                  //        var item = StockdataAdapter.records[cell.row];
                  ...
                  //},                  
                  {
                      text: "操作", datafield: "Edit", columntype: "button", width: "6%", cellsrenderer: function (row, columnfield, value, defaulthtml, columnproperties) {
                          //var item = StockdataAdapter.records[row];
                          //var isInArray = false;
                          ...
                          //    if (stocks[i].StockDetailId === item.StockDetailId) {
                          //        stocks[i].AlreadyNetAmount += item.NetAmount;
                          //        stocks[i].AlreadyNetAmount = Math.round((stocks[i].AlreadyNetAmount) * 1000) / 1000;

                          //        stocks[i].NetAmount -= item.NetAmount;
                          //        stocks[i].NetAmount = Math.round((stocks[i].NetAmount) * 1000) / 1000;
                          //    }
                          //}
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            repoStocks = RepodataAdapter.records;
            for (var i = 0; i < repoStocks.length; i++) {
                rowIds.push(repoStocks[i].StockDetailId);
            }
    ...
    <div style="text-align: center; width: 80%; margin-top: 0;">
        ...
        <a target="_self" runat="server" href="FinancingRepoApplyList.aspx" id="btnCancel" style="margin-left: 10px">取消</a>
    </div>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            function RepoApplyUpdate() {
                ...
                if(repoNetAmount > (Math.round((pledgeApplySumNetAmount-(aleadyNetAmount-parseFloat("<%=this.CurRepoApply.SumNetAmount%>"))) * 1000) / 1000)){
                    alert("赎回总净重不可大于 "+(Math.round((pledgeApplySumNetAmount-(aleadyNetAmount-parseFloat("<%=this.CurRepoApply.SumNetAmount%>"))) * 1000) / 1000));
                    return;
                }

                if (!confirm("确认修改赎回申请单？")) { return; }
                $("#btnSave").jqxButton({ disabled: true });

                var repoApply = {
                    RepoApplyId: "<%=this.CurRepoApply.RepoApplyId%>",
                    //RepoApplyIdNo: 
                    PledgeApplyId: "<%=this.CurPledgeApply.PledgeApplyId%>"
                    //SumNetAmount: 
                    //SumHands: 
                    //RepoApplyStatus: 
                };                

                $.post("Handler/FinRepoApplyUpdateHandler.ashx", { repoApply: JSON.stringify(repoApply), rows: JSON.stringify(rows) },
                    function (result) {
            ...
            $("#btnSave").click(function () { RepoApplyUpdate(); });
