<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="OrderList.aspx.cs" Inherits="NFMTSite.Document.OrderList" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#btnAddOrder").jqxLinkButton({ height: 25, width: 120 });
            ...
            var url = "Handler/OrderListHandler.ashx";
            var formatedData = "";
            var totalrecords = 0;
            var source =
            {
            ...
            var cellsrenderer = function (row, columnfield, value, defaulthtml, columnproperties) {
                ...
                if (item.OrderType == orderTypeEnum.临票制单指令 || item.OrderType == orderTypeEnum.终票制单指令) {
                    cellHtml += "<a target=\"_self\" href=\"OrderDetail.aspx?id=" + value + "\">明细</a>";
                }
                else if (item.OrderType == orderTypeEnum.无配货临票制单指令 || item.OrderType == orderTypeEnum.无配货终票制单指令) {
                    cellHtml += "<a target=\"_self\" href=\"NoStockOrderDetail.aspx?id=" + value + "\">明细</a>";
                }
                else if (item.OrderType == orderTypeEnum.替临制单指令) {
                    cellHtml += "<a target=\"_self\" href=\"OrderReplaceDetail.aspx?id=" + value + "\">明细</a>";
                }

                if (item.OrderStatus > statusEnum.已关闭 && item.OrderStatus < statusEnum.待审核) {
                    if (item.OrderType == orderTypeEnum.临票制单指令 || item.OrderType == orderTypeEnum.终票制单指令) {
                        cellHtml += "&nbsp;&nbsp;&nbsp<a target=\"_self\" href=\"OrderUpdate.aspx?id=" + value + "\">修改</a>";
                    }
                    else if (item.OrderType == orderTypeEnum.无配货临票制单指令 || item.OrderType == orderTypeEnum.无配货终票制单指令) {
                        cellHtml += "&nbsp;&nbsp;&nbsp<a target=\"_self\" href=\"NoStockOrderUpdate.aspx?id=" + value + "\">修改</a>";
                    }
                }
                else {
                    cellHtml += "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
            ...
            $("#btnSearch").click(function () {
                ...
                source.url = "Handler/OrderListHandler.ashx?cn=" + contractNo + "&ci=" + corpId + "&db=" + fromDate + "&de=" + toDate + "&s=" + status;
    ...
    <div id="jqxExpander" style="float: left; margin: 0px 5px 5px 5px; padding-bottom: 10px;">
        ...
        <div class="SearchExpander">
            <ul>
                ...
                <li>
                    <span style="float: left;">指令状态：</span>
                    <div id="selStatus" style="float: left;" />
                </li>

                <li>
                    <input type="button" id="btnSearch" value="查询" />
                    <a target="_self" runat="server" href="OrderContractList.aspx" id="btnAddOrder">新建制单指令</a>
                </li>
            </ul>
        </div>
    </div>
