<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="OrderReplaceCreate.aspx.cs" Inherits="NFMTSite.Document.OrderReplaceCreate" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $("#btnCreateAuditReplaceOrder").jqxInput();
            $("#btnCreateReplaceOrder").jqxInput();
        ...
        function CreateReplaceOrder(isAudit) {

            if (replaceStocks.length == 0) { alert("未选中任何库存"); return; }
            if (!confirm("确认新增替临制单指令?")) { return; }

            var Order = {
                ContractId: "<%= this.curOrder.ContractId%>",
                ContractNo:"<%= this.curOrder.ContractNo%>",
                SubId: "<%=this.curOrder.SubId%>",
                CommercialId: "<%=this.curOrder.OrderId%>",
                //LCId: 0,
                LCNo: "<%= this.curOrder.LCNo%>",
                LCDay: "<%= this.curOrder.LCDay%>",
                OrderType:"<%=this.ReplaceOrderType%>",
                OrderDate: $("#txbOrderDate").val(),
                ApplyCorp: "<%= this.curOrder.ApplyCorp%>",
                ApplyDept: "<%= this.curOrder.ApplyDept%>",
                BuyerCorp: "<%= this.curOrder.BuyerCorp%>",
                BuyerCorpName: "<%= this.curOrder.BuyerCorpName%>",
                BuyerAddress: "<%= this.curOrder.BuyerAddress%>",
                RecBankId: "<%= this.curOrder.RecBankId%>",
                DiscountBase: "<%= this.curOrder.DiscountBase%>",
                AssetId: "<%= this.curOrder.AssetId%>",
                BrandId: "<%= this.curOrder.BrandId%>",
                AreaName: "<%= this.curOrder.AreaName%>",
                BankCode: $("#txbBankCode").val(),
                GrossAmount: $("#txbGrossAmount").val(),
                NetAmount: $("#txbNetAmount").val(),
                UnitId: "<%= this.curOrder.UnitId%>",
                Bundles: $("#txbBundles").val(),
                Currency: "<%= this.curOrder.Currency%>",
            ...
            $.post("Handler/OrderReplaceCreateHandler.ashx", { order: JSON.stringify(Order), orderStockInvoice: JSON.stringify(replaceStocks), orderDetail: JSON.stringify(OrderDetail), IsSubmitAudit: isAudit },
                   function (result) {
...
<body>
    ...
    <div id="buttons" style="width: 80%; text-align: center; float: left;">
        <input type="button" value="新增替临指令并提交审核" id="btnCreateAuditReplaceOrder" onclick="CreateReplaceOrder(true);" style="width: 180px; height: 25px;" />
        <input type="button" value="新增替临指令" id="btnCreateReplaceOrder" onclick="CreateReplaceOrder(false);" style="width: 120px; height: 25px;" />
    </div>
