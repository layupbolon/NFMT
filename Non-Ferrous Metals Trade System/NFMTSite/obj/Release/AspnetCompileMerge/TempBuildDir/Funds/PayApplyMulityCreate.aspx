<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PayApplyMulityCreate.aspx.cs" Inherits="NFMTSite.Funds.PayApplyMulityCreate" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            formatedData = "";
            totalrecords = 0;
            var stockLogsUrl = "Handler/PayApplyMulityStockListHandler.ashx?subIds=" + "<%= this.subIds%>";           
            stockLogsSource =
            {
                datatype: "json",
                sort: function () {
            ...
            //申请公司
            var ownCorpUrl = "../User/Handler/CorpDDLHandler.ashx?isSelf=1";
            var ownCorpSource = { datatype: "json", url: ownCorpUrl, async: false };
            var ownCorpDataAdapter = new $.jqx.dataAdapter(ownCorpSource);
            $("#selApplyCorp").jqxComboBox({source: ownCorpDataAdapter, displayMember: "CorpName", valueMember: "CorpId", searchMode: "containsignorecase",selectedIndex:0});
            $("#selApplyCorp").jqxComboBox("val", "<%=this.applyCorpId%>");
            ...
            $("#selRecCorp").jqxComboBox({ source: outCorpDataAdapter, displayMember: "CorpName", valueMember: "CorpId", height: 25, searchMode: "containsignorecase" });
            ...
            $("#selRecCorp").jqxComboBox("val", "<%=this.outCorpId%>");
            ...
            //申请金额
            $("#txbApplyBala").jqxNumberInput({ height: 25, min: 0, max: 999999999, decimalDigits: 2, digits: 9, width: 140, spinButtons: true });
            ...
            function PayApplyCreate(isAudit) {
                ...
                $.post("Handler/PayApplyMultyCreateHandler.ashx", {
                    Apply: JSON.stringify(apply), 
                    PayApply: JSON.stringify(payApply), 
                    StockApps: JSON.stringify(stockApps),
                    RecBank:$("#txbBank").val(),
                    isAudit:isAudit
                },
                    function (result) {
...
function FlushGrid() {
    ...
    stockLogsSource.url = "Handler/PayApplyMulityStockListHandler.ashx?subIds=" + "<%= this.subIds%>" + "&logIds=" + logIds;
...
<body>
    ...
    <div id="jqxStockAppsExpander" style="float: left; margin: 0 5px 5px 5px;">
        <div>
            已选择付款库存
        </div>
        <div>
            <div id="jqxStockAppsGrid" style="float: left; margin: 5px 0 0 5px;"></div>
        </div>
    </div>

    <div id="jqxStockLogsExpander" style="float: left; margin: 0 5px 5px 5px;">
        <div>
            可申请付款库存
        </div>
        <div>
            <div id="jqxStockLogsGrid" style="float: left; margin: 5px 0 0 5px;"></div>
        </div>
    </div>

    <div id="jqxPayApplyExpander" style="float: left; margin: 0 5px 5px 5px;">
        <div>
            付款申请信息
        </div>
