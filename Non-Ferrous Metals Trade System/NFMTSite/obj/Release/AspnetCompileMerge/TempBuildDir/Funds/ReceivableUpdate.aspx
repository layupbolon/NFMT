<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ReceivableUpdate.aspx.cs" Inherits="NFMTSite.Funds.ReceivableUpdate" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#ddlReceivableCorpId").jqxComboBox({ source: ddlCorpIddataAdapter, displayMember: "CorpName", valueMember: "CorpId", width: 260, height: 25 });            

            $("#ddlReceivableCorpId").on("change", function (event) {
                var args = event.args;
                if (args != undefined) {
            ...
            $("#ddlReceivableBank").on("change", function (event) {
                var args = event.args;
                if (args != undefined) {
                    var item = event.args.item;
                    if (item != null) {
            ...
            $("#ddlCurrencyId").jqxComboBox({ source: ddlMUIddataAdapter, displayMember: "CurrencyName", valueMember: "CurrencyId", width: 260, height: 25, autoDropDownHeight: true });            
            //付款公司
            ...
            $("#ddlPayCorpId").on("change", function (event) {
                var args = event.args;
                if (args != undefined) {
                    var item = event.args.item;
                    if (item != null) {
            ...
            $("#ddlPayBankId").on("change", function (event) {
                var args = event.args;
                if (args != undefined) {
                    var item = event.args.item;
                    if (item != null) {
                        ...
                        var ddlBankAccountsource = { datatype: "json", datafields: [{ name: "BankAccId" }, { name: "AccountNo" }], url: ddlBankAccounturl, async: false };
                        var ddlBankAccountdataAdapter = new $.jqx.dataAdapter(ddlBankAccountsource);
                        $("#ddlPayAccountId").jqxComboBox({ source: ddlBankAccountdataAdapter, displayMember: "AccountNo", valueMember: "BankAccId", width: 180, height: 25 });
                    }
                }
            });
            ...
            //控件赋值
            $("#dtReceiveDate").jqxDateTimeInput("val", "<%=this.curReceivable.ReceiveDate.ToShortDateString()%>");
            $("#ddlReceivableCorpId").jqxComboBox("val", <%=this.curReceivable.ReceivableCorpId%>);
            $("#ddlReceivableBank").jqxComboBox("val", <%=this.curReceivable.ReceivableBank%>);
            $("#ddlReceivableAccoontId").jqxComboBox("val", <%=this.curReceivable.ReceivableAccoontId%>);
            $("#nbPayBala").jqxNumberInput("val", <%=this.curReceivable.PayBala%>);
            $("#ddlCurrencyId").jqxComboBox("val",<%=this.curReceivable.CurrencyId%>);

            if(<%=this.curReceivable.PayCorpId%>>0){
                $("#ddlPayCorpId").jqxComboBox("val", <%=this.curReceivable.PayCorpId%>);
            }
            $("#txtPayCorp").val("<%=this.curReceivable.PayCorpName%>");

            if(<%=this.curReceivable.PayBankId%>>0){
                $("#ddlPayBankId").jqxComboBox("val", <%=this.curReceivable.PayBankId%>);
            }
            $("#txtPayBank").val("<%=this.curReceivable.PayBank%>");

            if(<%=this.curReceivable.PayAccountId%>>0){
                $("#ddlPayAccountId").jqxComboBox("val", <%=this.curReceivable.PayAccountId%>);
            }
            $("#txtPayAccount").val("<%=this.curReceivable.PayAccount%>");
            $("#txbPayWord").val("<%=this.curReceivable.PayWord%>");
            $("#txtBankLog").val("<%=this.curReceivable.BankLog%>");
            ...
            $("#btnUpdate").on("click", function () {
                ...
                //alert(isNaN($("#ddlPayCorpId").val()));

                var receivableModel = {
                    ReceivableId: $("#hidid").val(),
                    //ReceiveEmpId
                    ...
                    PayAccountId: $("#ddlPayAccountId").val() ? $("#ddlPayAccountId").val():0,
                    PayAccount: $("#txtPayAccount").val(),
                    BankLog: $("#txtBankLog").val()
                };

                $.post("Handler/ReceivableUpdateHandler.ashx", { Receivable: JSON.stringify(receivableModel) },
                    function (result) {
                        alert(result);
        ...
        <div id="layOutDiv">
            <ul>
                ...
                <li>
                    ...
                    <input type="text" id="txtPayCorp" style="float: left;" runat="server" />
                </li>

                <li>
                    <span style="text-align: right; width: 15%;">付款银行：</span>
                    <div id="ddlPayBankId" style="float: left;"></div>
                    <input type="text" id="txtPayBank" style="float: left;" runat="server" />

                    <span style="text-align: right; width: 15%;">付款账户：</span>
                    <div id="ddlPayAccountId" style="float: left;"></div>
                    <input type="text" id="txtPayAccount" style="float: left;" runat="server" />
                </li>

                <li>
                    <span style="text-align: right; width: 15%;">简短附言：</span>
                    <input type="text" id="txbPayWord" style="float: left;" runat="server" />

                    <span style="text-align: right; width: 15%;">外部流水备注：</span>
                    <input type="text" id="txtBankLog" style="float: left;" runat="server" />
                </li>
