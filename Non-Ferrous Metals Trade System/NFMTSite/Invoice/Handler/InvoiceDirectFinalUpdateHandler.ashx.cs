...
{
    /// <summary>
    /// InvoiceDirectFinalUpdateHandler 的摘要说明
    /// </summary>
    public class InvoiceDirectFinalUpdateHandler : IHttpHandler
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                result = bll.UpdateDirect(user, invoice, invoiceBusiness, details);

                if (result.ResultStatus == 0)
                {
                    result.Message = "直接终票修改成功";
                }
            }
            catch (Exception ex)
            {
