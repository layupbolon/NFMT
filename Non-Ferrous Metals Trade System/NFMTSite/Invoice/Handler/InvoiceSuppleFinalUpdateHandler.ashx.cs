...
{
    /// <summary>
    /// InvoiceSuppleFinalUpdateHandler 的摘要说明
    /// </summary>
    public class InvoiceSuppleFinalUpdateHandler : IHttpHandler
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                result = bll.UpdateSuppleFinal(user, invoice, invoiceBusiness, details);

                if (result.ResultStatus == 0)
                {
                    result.Message = "补零终票修改成功";
                }
            }
            catch (Exception ex)
            {
