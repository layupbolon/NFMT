...
{
    /// <summary>
    /// FinanceInvoiceUpdateHandler 的摘要说明
    /// </summary>
    public class FinanceInvoiceUpdateHandler : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                result = bll.Update(user, invoice, fundsInvoice, iids);

                if (result.ResultStatus == 0)
                {
                    result.Message = "财务发票修改成功";
                }
            }
            catch (Exception ex)
            {
