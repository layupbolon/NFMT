...
{
    /// <summary>
    /// InvoiceReplaceFinalCreateHandler 的摘要说明
    /// </summary>
    public class InvoiceReplaceFinalCreateHandler : IHttpHandler
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                result = bll.CreateReplaceFinal(user, invoice, invoiceBusiness, details);

                if (result.ResultStatus == 0)
                {
                    result.Message = "替临终票新增成功";
                }
            }
            catch (Exception ex)
            {
