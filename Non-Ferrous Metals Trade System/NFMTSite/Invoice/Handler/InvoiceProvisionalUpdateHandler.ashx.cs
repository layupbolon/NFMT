...
{
    /// <summary>
    /// InvoiceProvisionalUpdateHandler 的摘要说明
    /// </summary>
    public class InvoiceProvisionalUpdateHandler : IHttpHandler
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                result = bll.UpdateProvisional(user, invoice, invoiceBusiness, details);               

                if (result.ResultStatus == 0)
                {
                    result.Message = "临票修改成功";
                }
            }
            catch (Exception ex)
            {
