...
{
    /// <summary>
    /// InvoiceSuppleFinalCreateHandler 的摘要说明
    /// </summary>
    public class InvoiceSuppleFinalCreateHandler : IHttpHandler
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                result = bll.CreateSuppleFinal(user, invoice, invoiceBusiness, details);

                if (result.ResultStatus == 0)
                {
                    result.Message = "补零终票新增成功";
                }
            }
            catch (Exception ex)
            {
