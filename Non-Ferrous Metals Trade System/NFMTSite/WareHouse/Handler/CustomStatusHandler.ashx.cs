...
{
    /// <summary>
    /// CustomStatusHandler 的摘要说明
    /// </summary>
    public class CustomStatusHandler : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (!int.TryParse(context.Request.Form["id"], out customId) || customId <= 0)
            {
            ...
            switch (operateEnum)
            {
                case NFMT.Common.OperateEnum.撤返:
                    result = bll.GoBack(user, customId);
                    break;
                case NFMT.Common.OperateEnum.作废:
                    result = bll.Invalid(user, customId);
                    break;
                case NFMT.Common.OperateEnum.执行完成:
                    result = bll.Complete(user, customId);
                    break;
                case NFMT.Common.OperateEnum.执行完成撤销:
                    result = bll.CompleteCancel(user, customId);
                    break;
                case NFMT.Common.OperateEnum.关闭:
                    result = bll.Close(user, customId);
                    break;
            }
