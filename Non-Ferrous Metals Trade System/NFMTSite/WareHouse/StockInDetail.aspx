<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="StockInDetail.aspx.cs" Inherits="NFMTSite.WareHouse.StockInDetail" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        var auditUrl = "../WorkFlow/Handler/AuditProgressListHandler.ashx?b=" + "<%=this.curStockIn.DataBaseName%>" + "&t=" + "<%=this.curStockIn.TableName%>" + "&id=" + "<%=this.curStockIn.StockInId%>";
        ...
        $(document).ready(function () {
            ...
            $("#btnComplete").on("click", function () {
                ...
                $.post("Handler/StockInStatusHandler.ashx", { id: "<%=this.curStockIn.StockInId%>",oi: operateEnum.执行完成 },
                    function (result) {
                        var obj = JSON.parse(result);
                        alert(obj.Message);   
                        if (obj.ResultStatus.toString() == "0") {
            ...
            $("#btnCompleteCancel").on("click", function () {
                ...
                $.post("Handler/StockInStatusHandler.ashx", { id: "<%=this.curStockIn.StockInId%>",oi: operateEnum.执行完成撤销 },
                    function (result) {
                        var obj = JSON.parse(result);
                        alert(obj.Message);   
                        if (obj.ResultStatus.toString() == "0") {
            ...
            $("#btnClose").on("click", function () {
                if (!confirm("确认完成撤销？")) { return; }
                $.post("Handler/StockInStatusHandler.ashx", { id: "<%=this.curStockIn.StockInId%>",oi: operateEnum.关闭 },
                    function (result) {
                        var obj = JSON.parse(result);
                        alert(obj.Message);   
                        if (obj.ResultStatus.toString() == "0") {
