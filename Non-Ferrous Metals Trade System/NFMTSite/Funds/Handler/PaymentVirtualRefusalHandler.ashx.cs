...
{
    /// <summary>
    /// PaymentVirtualRefusalHandler 的摘要说明
    /// </summary>
    public class PaymentVirtualRefusalHandler : IHttpHandler
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                result = bll.Close(user, paymentVirtual.VirtualId);

                if (result.ResultStatus == 0)
                {
                    result.Message = "虚拟付款拒绝成功";
                }
            }
            catch (Exception ex)
            {
