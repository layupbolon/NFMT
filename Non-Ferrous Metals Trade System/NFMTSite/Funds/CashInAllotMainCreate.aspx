<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CashInAllotMainCreate.aspx.cs" Inherits="NFMTSite.Funds.CashInAllotMainCreate" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $("#dtDateBg").jqxDateTimeInput({ formatString: "yyyy-MM-dd", width: 120 });
            var tempDate = new Date();
            tempDate.setDate(1);
            $("#dtDateBg").jqxDateTimeInput({ value: tempDate });
            $("#dtDateEnd").jqxDateTimeInput({ formatString: "yyyy-MM-dd", width: 120 });
            var date=new Date();
            var currentMonth=date.getMonth();
            var nextMonth=++currentMonth;
            var nextMonthFirstDay=new Date(date.getFullYear(),nextMonth,1);
            var oneDay=1000*60*60*24;
            $("#dtDateEnd").jqxDateTimeInput({ value: new Date(nextMonthFirstDay-oneDay) });

            $("#btnSearchContract").jqxButton({ width: 80 });

            var fromDate = $("#dtDateBg").jqxDateTimeInput("getText");
            var toDate = $("#dtDateEnd").jqxDateTimeInput("getText");
            var formatedData = "";
            var totalrecords = 0;
            var source =
            {
                ...
                url: "Handler/CashInAllotMainCreateContractListHandler.ashx?s=" + statusEnum.已生效 + "&ci=<%=this.cashIn.PayCorpId%>" + "&db=" + fromDate + "&de=" + toDate
        };
            ...
            var cellsrenderer = function (row, columnfield, value, defaulthtml, columnproperties) {
                return "<span style=\"padding-left:20px;\"><input type=\"radio\" value=\"" + value + "\" name=\"radCon\" /></span>";
            }
            ...
            $("#jqxContractGrid").jqxGrid(
            {
                ...
                columns: [
                  { text: "选择子合约", datafield: "SubId", cellsrenderer: cellsrenderer, width: 77, enabletooltips: false },
                  { text: "签订日期", datafield: "ContractDate", cellsformat: "yyyy-MM-dd" ,width:"10%"},
                  { text: "内部子合约号", datafield: "SubNo",width:"10%" },
                  { text: "外部合约号", datafield: "OutContractNo",width:"10%"  },
                  { text: "购销方向", datafield: "TradeDirectionName",width:"10%"  },
                  { text: "我方公司", datafield: "InCorpName" ,width:"10%" },
                  { text: "对方公司", datafield: "OutCorpName",width:"15%" },
                  { text: "交易品种", datafield: "AssetName",width:"7%" },
                  { text: "合约重量", datafield: "ContractWeight" ,width:"10%"},
                  { text: "点价方式", datafield: "PriceModeName" ,width:"9%"},
                  { text: "合约状态", datafield: "StatusName",width:"9%" }
                ]
            }).on("bindingcomplete", function (event) {
                var rads = $("input[name=\"radCon\"]");
                for (i = 0; i < rads.length; i++) {
                    var rad = $(rads[i]);
                    var val = rad.val();
                    rad.click({ value: val }, function (event) {
                        subContractId = event.data.value;

                        //更新库存
                        $.get("Handler/CashInAllotMainContractStockListHandler.ashx?cid=" + subContractId, function (data) {
                            invStocksSource.localdata = data;
                            $("#jqxStockGrid").jqxGrid("updatebounddata", "rows");

                            //更新合约库存价格
                            $.get("Handler/CashInAllotMainGetContractPriceHandler.ashx?cid=" + subContractId, function (result) { 
                                $("#nbStockPrice").jqxNumberInput("val", 0); 
                                $("#nbStockPrice").jqxNumberInput("val", result); });
                        });

                        //更新公司
                        allotCorpUrl = "../User/Handler/CorpDDLHandler.ashx?isSelf=0&SubId=" + subContractId;
                        allotCorpSource = { datatype: "json", url: allotCorpUrl, async: false };
                        allotCorpDataAdapter = new $.jqx.dataAdapter(allotCorpSource);
            ...
            //库存单价nbStockPrice
            $("#nbStockPrice").jqxNumberInput({ height: 25, min: 0, decimalDigits: 2, width: 140, spinButtons: true, symbolPosition: "right", symbol: "<%=this.CurrencyName%>" });
            $("#nbStockPrice").on("valueChanged", function (event) {
                var value = event.args.value;
                var lastBala = <%=this.CanAllotBala%>;
                ...
                if(rows.length > 0 && hasData){
                    ...
                    for (i = 0; i < rows.length; i++) {
                        ...
                        var curBala = Math.round((item.NetGapAmount * value) * 100) / 100;
                        var curAmount = item.NetGapAmount;
                        ...
                        if(lastBala >= curBala){
                            ...
                            curBala = lastBala;
                            lastBala =0;
                            curAmount = Math.round((curBala/value) * 10000)/10000;
                        }

                        item.AllotBala = Math.round(curBala *100)/100;
                        item.AllotNetAmount = Math.round(curAmount* 10000)/10000;
                    ...
                    //更新Grid
                    invStocksSource.localdata = sourceRows;
                    $("#jqxStockGrid").jqxGrid("updatebounddata", "rows");
            ...
            $.get("Handler/CashInAllotMainContractStockListHandler.ashx?cid=0", function (result) {
                localdataSource = result;
            });
            ...
            invStocksSource =
            {
                ...
                //type: "GET",
                //url: "Handler/CashInAllotMainContractStockListHandler.ashx?cid=0"
            ...
            $("#jqxStockGrid").on("cellvaluechanged", function (event) {                              
                var sumBala = 0;
                var rows = invStocksDataAdapter.records;
                //alert(invStocksDataAdapter.records);
                for (i = 0; i < rows.length; i++) {
                    var item = rows[i];
                    if (item.StockId > 0) {
                        sumBala += item.AllotBala;
                ...
                //更新控件  默认分配金额取 收款登记金额与合约金额（库存单价*数量）的较小值
                $("#nbAllotBala").val(sumBala);                    
            });
                ...
                columns: [
                  ...
                  { text: "分配金额", datafield: "AllotBala" },
                  {
                      text: "操作", datafield: "Edit", columntype: "button", width: 90, cellsrenderer: function () {
                          ...
                          $("#jqxSIInvoiceGrid").jqxGrid("deleterow", row);
                      }
                  }
                ]
            });
            ...
            var CanApplySIsource =
            {
                ...
                url: "../Invoice/Handler/InvoiceApplySISelectedListHandler.ashx?sIIds=<%=this.sIIds%>"
            }
            ...
            //分配公司
            var allotCorpUrl = "../User/Handler/CorpDDLHandler.ashx?isSelf=0&SubId=" + subContractId;
            ...
            $("#ddlCurrency").jqxDropDownList("val", "<%=this.cashIn.CurrencyId%>");
            ...
            $("#btnCreate").click(function () {
                ...
                $.post("Handler/CashInAllotMainCreateHandler.ashx", {
    ...
    <div id="jqxContractExpander" style="float: left; margin: 0px 5px 5px 5px;">
        ...
        <div>
            <div class="InfoExpander">
                <ul>
                    <li>
                        <strong style="float: left;margin-left:5px;">签订日期：</strong>
                        <div id="dtDateBg" style="float: left;"></div>
                    </li>
                    <li>
                        <strong style="float: left;margin-left:5px;">到：</strong>
                        <div id="dtDateEnd" style="float: left;"></div>
                    </li>
                    <li>
                        <input type="button" value="查询" id="btnSearchContract" style="margin-left:10px;" />
                    </li>
                </ul>
            </div>
    ...
    <div id="jqxStockExpander" style="float: left; margin: 0px 5px 5px 5px;">
        ...
        <div>
            <div class="DataExpander">
                <ul>
                    <li>
                        <strong style="float: left;">库存单价：</strong>
                        <div id="nbStockPrice" style="margin-left: 5px; margin-bottom: 5px;"></div>
                    </li>
                </ul>
            </div>
