...
{
    public partial class PaymentStockUpdate : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (payment == null || payment.PaymentId <= 0)
                    Response.Redirect(redirectUrl);
                if (payment.PaymentStatus < NFMT.Common.StatusEnum.已录入 || payment.PaymentStatus >= NFMT.Common.StatusEnum.已生效)
                    Response.Redirect(redirectUrl);
                ...
                {
                    this.curPaymentVirtual = new NFMT.Funds.Model.PaymentVirtual();
                }
