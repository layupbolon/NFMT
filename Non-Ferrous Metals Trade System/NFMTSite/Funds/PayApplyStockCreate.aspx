<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PayApplyStockCreate.aspx.cs" Inherits="NFMTSite.Funds.PayApplyStockCreate" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $("#jqxSelectGrid").jqxGrid(
            {
                ...
                columns: [
                  ...
                  { text: "操作", datafield: "StockId", cellsrenderer: removeRender, enabletooltips: false, sortable: false, editable: false }
                ]
            });
            ...
            stockListSource =
            {
                ...
                url: "Handler/PayApplyStockListHandler.ashx"
            };
            var stockListDataAdapter = new $.jqx.dataAdapter(stockListSource, {
                contentType: "application/json; charset=utf-8",
                loadError: function (xhr, status, error) {
                    alert(error);
                }
                ...
                columns: [
                  ...
                  { text: "库存状态", datafield: "StockStatusName", editable: false },
                  {
                      text: "申请金额", datafield: "ApplyBala", columntype: "numberinput", cellclassname: "GridFillNumber", width: 120, sortable: false, validation: function (cell, value) {
                          if (value <= 0) {
                              return { result: false, message: "申请金额必须大于0" };
                          }
                          return true;
                  ...
                  { text: "操作", datafield: "StockId", cellsrenderer: addRender, enabletooltips: false, sortable: false, editable: false }
                ]
            });
            ...
            $("#btnCreate").click(function () {
                ...
                if (selects.length == 0) { alert("未选中任务库存"); return; }
                ...
                for (i = 0; i < selects.length; i++) {
                    stocks += "{ StockId:" + selects[i].StockId + ",ApplyBala:" + selects[i].ApplyBala + ",StockLogId:" + selects[i].StockLogId + " }";
                    if (i < selects.length - 1) { stocks += "|"; }
                }

                $.post("Handler/PayApplyStockCreateHandler.ashx", { PayApply: JSON.stringify(payApply), Stocks: stocks, memo: $("#txbMemo").val(), deptId: $("#selApplyDept").val(), corpId: $("#selApplyCorp").jqxComboBox("val") },
                    function (result) {
        ...
        function bntAddOnClick(row) {
            ...
            var item = $("#jqxStockListGrid").jqxGrid("getrowdatabyid", row);
            if (isNaN(item.ApplyBala)) {
                alert("必须填写申请金额");
                return;
            }
            ...
            for (i = 0; i < selects.length; i++) {
                var temp = selects[i];
                if (!isNaN(temp.StockId)) {
                    if (i > 0) { sids += ","; }
                    sids += temp.StockId;
                }
            }
            stockListSource.url = "Handler/PayApplyStockListHandler.ashx?sids=" + sids;
        ...
        function bntRemoveOnClick(row) {
            ...
            var item = $("#jqxSelectGrid").jqxGrid("getrowdatabyid", row);
            var index = selects.indexOf(item);
