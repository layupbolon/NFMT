...
{
    /// <summary>
    /// AuthGroupListHandler 的摘要说明
    /// </summary>
    public class AuthGroupListHandler : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            int status = 0;
            if (string.IsNullOrEmpty(context.Request["s"]) || !int.TryParse(context.Request["s"], out status))
                status = 0;
            ...
            NFMT.Common.SelectModel select = bll.GetSelectModel(pageIndex, pageSize, orderStr, name, assetId, tradeDirection, tradeBorder, contractInOut, contractLimit, corpId, status);
