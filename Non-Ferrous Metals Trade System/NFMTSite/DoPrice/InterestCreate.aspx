<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="InterestCreate.aspx.cs" Inherits="NFMTSite.DoPrice.InterestCreate" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $("#selInertestStyle").jqxComboBox({ source: inertestStyleDataAdapter, displayMember: "DetailName", valueMember: "StyleDetailId", height: 25, autoDropDownHeight: true,selectedIndex:0 }); 

            //合同剩余本金
            $("#txbPayCapital").jqxNumberInput({ height: 25, spinButtons: true, decimalDigits: 2, Digits: 9, symbol: "<%=this.cur.CurrencyName%>", symbolPosition: "right" });
            $("#txbPayCapital").val(<%=this.lastPayCapital%>);
            
            //合同数量
            $("#txbContractAmount").jqxNumberInput({ height: 25, spinButtons: true, decimalDigits: 4, Digits: 9, symbol: "<%=this.mu.MUName%>", symbolPosition: "right", disabled: true });
            $("#txbContractAmount").jqxNumberInput("val", "<%=this.sub.SignAmount%>");

            //发货数量
            $("#txbInterestAmount").jqxNumberInput({ height: 25, spinButtons: true, decimalDigits: 4, Digits: 9, symbol: "<%=this.mu.MUName%>", symbolPosition: "right",disabled: true });
            $("#txbInterestAmount").val(<%=this.netWeight%>);

            //利息合计
            $("#txbInterestBala").jqxNumberInput({ height: 25, spinButtons: true, decimalDigits: 2, Digits: 9, symbol: "<%=this.cur.CurrencyName%>", symbolPosition: "right" });

            //期货点价
            $("#txbPricingUnit").jqxNumberInput({ height: 25, spinButtons: true, decimalDigits: 4, Digits: 9, symbol: "<%=this.cur.CurrencyName%>", symbolPosition: "right" });

            //升贴水
            $("#txbPremium").jqxNumberInput({ height: 25, spinButtons: true, decimalDigits: 4, Digits: 9, symbol: "<%=this.cur.CurrencyName%>", symbolPosition: "right", disabled: true });
            $("#txbPremium").jqxNumberInput("val", "<%=this.sub.Premium%>");

            //其他价格
            $("#txbOtherPrice").jqxNumberInput({ height: 25, spinButtons: true, decimalDigits: 4, Digits: 9, symbol: "<%=this.cur.CurrencyName%>", symbolPosition: "right" });

            //计息单价
            $("#txbInterestPrice").jqxNumberInput({ height: 25, spinButtons: true, decimalDigits: 4, Digits: 9, symbol: "<%=this.cur.CurrencyName%>", symbolPosition: "right",disabled: true });

            //利息率
            $("#txbInterestRate").jqxNumberInput({ height: 25, spinButtons: true, decimalDigits: 4, Digits: 3, symbol: "%", symbolPosition: "right" });            

            //每吨天利息
            $("#txbInterestAmountDay").jqxNumberInput({ height: 25, spinButtons: true, decimalDigits: 4, Digits: 9, symbol: "<%=this.cur.CurrencyName%>", symbolPosition: "right" });            

            //备注
            $("#txbMemo").jqxInput({ width: "99%", height: 25 });
            
            $("#btnAdd").jqxButton({ height: 25, width: 100 });
            $("#btnAdd").click(function () { PriceConfirmCreate(false); });
            $("#btnAduit").jqxButton({ width: 180, height: 25 });
            $("#btnAduit").click(function () { PriceConfirmCreate(true); });
                ...
                columns: [
                  ...
                  { text: "计息天数", datafield: "InterestDay", width: 65, editable: false },
                  {
                      text: "日利息额", datafield: "InterestUnit", columntype: "numberinput", cellclassname: "GridFillNumber", cellsrenderer: function (row, columnfield, value, defaulthtml, columnproperties) {
                          return value +  "<%=this.cur.CurrencyName%>";
                      },
                      createeditor: function (row, cellvalue, editor) {
                  ...
                  { text: "操作", datafield: "StockLogId", width: 60, cellsrenderer: removeRender, enabletooltips: false, sortable: false, editable: false }
                ]
            });
            ...
            $("#jqxSubStockLogsGrid").jqxGrid(
            {
                ...
                columns: [
                  ...
                  { text: "操作", datafield: "StockLogId", width: 60,cellsrenderer: addRender, enabletooltips: false, sortable: false, editable: false }
                ]
            });
            ...
            function PriceConfirmCreate(isAudit) {
                var isCanSubmit = $("#jqxExpander").jqxValidator("validate");
                if (!isCanSubmit) { return; }

                if(!confirm("确认新增利息结算？")){ return ;}

                $("#btnAdd").jqxButton({ disabled: true });

                var interest = {                    
                    SubContractId: "<%=this.sub.SubId%>",
                    ...
                    Unit: "<%=this.sub.UnitId%>",
                    InterestStyle:$("#selInertestStyle").val(),
                    Memo: $("#txbMemo").val(),
                ...
                //判断利息明细与利息总额
                if(Math.round(sumDetailInterest*100)/100 != Math.round($("#txbInterestBala").val()*100)/100){
                ...
                $.post("Handler/InterestCreateHandler.ashx", { Interest: JSON.stringify(interest),InterestDetail: JSON.stringify(interestDetail),IsSubmitAudit: isAudit },function (result) {
