...
{
    public partial class PricingDetail : System.Web.UI.Page
    {
        public int mUId = 0;
        ...
        public NFMT.DoPrice.Model.Pricing pricing = new NFMT.DoPrice.Model.Pricing();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                ver.JudgeOperate(this.Page, 60, new List<NFMT.Common.OperateEnum>() { NFMT.Common.OperateEnum.提交审核, NFMT.Common.OperateEnum.作废, NFMT.Common.OperateEnum.撤返, NFMT.Common.OperateEnum.执行完成, NFMT.Common.OperateEnum.执行完成撤销 });

                this.hidSummaryPrice.Value = ((int)NFMT.Data.StyleEnum.SummaryPrice).ToString();

                this.navigation1.Routes.Add("点价单", redirectUrl);
                this.navigation1.Routes.Add("点价单明细", string.Empty);
                ...
                NFMT.DoPrice.BLL.PricingDetailBLL pricingDetailBLL = new NFMT.DoPrice.BLL.PricingDetailBLL();

                result = pricingDetailBLL.GetPricingApplyDetailIds(user, pricingId);
                //if (result.ResultStatus != 0)
                //    Response.Redirect(redirectUrl);
                this.hidDetailsIdDown.Value = result.ReturnValue != null ? result.ReturnValue.ToString() : string.Empty;
                ...
                if (mUId != 0)
                {
                    NFMT.Data.Model.MeasureUnit measureUnit = NFMT.Data.BasicDataProvider.MeasureUnits.SingleOrDefault(a => a.MUId == mUId);
                    if (measureUnit != null)
                        curMUName = measureUnit.MUName;
                }
