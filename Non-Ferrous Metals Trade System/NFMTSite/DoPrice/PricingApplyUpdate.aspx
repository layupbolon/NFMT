<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PricingApplyUpdate.aspx.cs" Inherits="NFMTSite.DoPrice.PricingApplyUpdate" %>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            var selectSource =
            {
                ...
                url: "Handler/PricingApplyUpdateStockListHandler.ashx?pricingApplyId=" + "<%=this.pricingApply.PricingApplyId%>"
            };
            ...
            $("#ddlPricingCorpId").jqxComboBox({ source: outCorpDataAdapter, displayMember: "CorpName", valueMember: "CorpId", height: 25 });
            ...
            $("#ddlPricingStyle").jqxDropDownList({ source: summaryPriceDataAdapter, displayMember: "DetailName", valueMember: "StyleDetailId", width: 100, height: 25, autoDropDownHeight: true });
            ...
            if ("<%=this.pricingApply.PricingStyle%>" == 146) {
                $(".hiddenClass").show();
            }
            else if ("<%=this.pricingApply.PricingStyle%>" == 147) {
                $(".hiddenClass").hide();
            }
            ...
            $("#jqxPricingApplyExpander").jqxValidator({
                rules:
                    [
                        ...
                        {
                            input: "#nbPricingWeight", message: "点价重量必须大于0且不大于" + "<%=this.curContractSub.SignAmount - this.alreadyPricingWeight+this.pricingApply.PricingWeight%>", action: "keyup,blur", rule: function (input, commit) {
                                return $("#nbPricingWeight").jqxNumberInput("val") > 0 && $("#nbPricingWeight").jqxNumberInput("val") <= "<%=this.curContractSub.SignAmount - this.alreadyPricingWeight+this.pricingApply.PricingWeight%>";
                            }
                        },
                        {
                            input: "#ddlPricingStyle", message: "点价方式必选", action: "change", rule: function (input, commit) {
                                return $("#ddlPricingStyle").jqxDropDownList("val") > 0;
                            }
                        }
                    ]
            });
            ...
            $("#btnUpdate").click(function () {
                ...
                for (i = 0; i < rows.length; i++) {
                    var item = rows[i];
                    if (item.PricingWeight == undefined || item.PricingWeight == null) {
                        item.PricingWeight = 0;
                    }
                }

                $.post("Handler/PricingApplyUpdateHandler.ashx", {
...
<body>
    ...
    <div style="width: 80%; text-align: center;">
        ...
        <a target="_self" runat="server" href="PricingApplyList.aspx" id="btnCancel">取消</a>&nbsp;&nbsp;&nbsp;&nbsp;
    </div>
