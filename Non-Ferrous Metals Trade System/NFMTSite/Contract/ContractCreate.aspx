<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ContractCreate.aspx.cs" Inherits="NFMTSite.Contract.ContractCreate" %>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("#selPriceFrom").jqxDropDownList({ source: exChangeDataAdapter, displayMember: "ExchangeName", valueMember: "ExchangeId", width: 100, height: 25, autoDropDownHeight: true, selectedIndex: 0 });
        ...
        <%=this.contractBLL.GetListBoxsInit(this.user, NFMT.Contract.BLL.ContractBLL.OperateEnum.新增,0)%>
        ...
        function ContractCreate(isAudit) {
            ...
            $.post("Handler/ContractCreateHandler.ashx", { Contract: JSON.stringify(contract), ContractDetail: JSON.stringify(contractDetail), ContractPrice: JSON.stringify(contractPrice), OutCorps: outCorps, InCorps: inCorps, Depts: depts, IsSubmitAudit: isAudit, checkedClause: JSON.stringify(eval(checkedClause)) },
                function (result) {
                    ...
                    if (obj.ResultStatus.toString() == "0") {
                        if (isAudit) {
                            AutoSubmitAudit(MasterEnum.合约审核, JSON.stringify(obj.ReturnValue));
                        }
                    }
                    alert(obj.Message);
                    if (obj.ResultStatus.toString() == "0") {
