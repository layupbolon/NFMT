<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SubList.aspx.cs" Inherits="NFMTSite.Contract.SubList" %>
        ...
        $(document).ready(function () {
            ...
            var cellsrenderer = function (row, columnfield, value, defaulthtml, columnproperties) {
                ...
                if (item.CreateFrom == "<%=(int)NFMT.Common.CreateFromEnum.采购合约库存创建%>") {
                    cellHtml += "<a target=\"_self\" href=\"ContractInDetail.aspx?sid=" + value + "\">明细</a>";
                }
                    ...
                    cellHtml += "<a target=\"_self\" href=\"SubDetail.aspx?id=" + value + "\">明细</a>";
                }
                if (item.SubStatus > statusEnum.已关闭 && item.SubStatus < statusEnum.待审核) {
                    if (item.CreateFrom == "<%=(int)NFMT.Common.CreateFromEnum.采购合约库存创建%>") {
                        cellHtml += "&nbsp;&nbsp;&nbsp<a target=\"_self\" href=\"ContractInUpdate.aspx?sid=" + value + "\">修改</a>";
                    }
                    else if (item.CreateFrom == "<%=(int)NFMT.Common.CreateFromEnum.销售合约库存创建%>") {
                        cellHtml += "&nbsp;&nbsp;&nbsp<a target=\"_self\" href=\"ContractOutUpdate.aspx?id=" + value + "\">修改</a>";
                    }
                    else {
                        cellHtml += "&nbsp;&nbsp;&nbsp<a target=\"_self\" href=\"SubUpdate.aspx?id=" + value + "\">修改</a>";
                    }
                }
                else {
                    cellHtml += "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
            ...
            $("#btnAddContractSub").jqxLinkButton({ height: 25, width: 120 });
        });
    </script>
</head>
<body>
        ...
        <div class="SearchExpander">
            <ul>
                ...
                <li>
                    ...
                    <a target="_self" runat="server" href="ContractSubList.aspx" id="btnAddContractSub">新建子合约</a>
                </li>
            </ul>
        </div>
    </div>
