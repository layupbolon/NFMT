...
namespace NFMT.StockBasic.BLL
{
    ...
    public class StockLogBLL : Common.ExecBLL
    {
        ...
        /// <returns></returns>
        public NFMT.Common.ResultModel StockHandle(NFMT.Common.UserModel user, NFMT.StockBasic.Model.StockLog stockLog, List<NFMT.StockBasic.Model.StockLogAttach> stockLogAttachs, StockOperateEnum stockOperate)
        {
            ...
            {
                //获取stock
                NFMT.StockBasic.BLL.StockBLL stockBLL = new NFMT.StockBasic.BLL.StockBLL();
                result = stockBLL.Get(user, stockLog.StockId);
                if (result.ResultStatus != 0)
                    return result;

                NFMT.StockBasic.Model.Stock stock = result.ReturnValue as NFMT.StockBasic.Model.Stock;
                ...
                //写库存流水
                NFMT.StockBasic.DAL.StockLogDAL stockLogDAL = new StockLogDAL();
                ...
                {
                    NFMT.StockBasic.DAL.StockLogAttachDAL stockLogAttachDAL = new StockLogAttachDAL();
                    foreach (NFMT.StockBasic.Model.StockLogAttach stockLogAttach in stockLogAttachs)
                    {
        ...
        public SelectModel GetLogsBySubIdSelect(int pageIndex, int pageSize, string orderStr, int subId, string logIds, string refIds)
        {
            ...
            sb.AppendFormat(" left join NFMT.dbo.Fun_StockPayApply_Ref spa on spa.StockLogId = sl.StockLogId and spa.RefId not in ({0}) ", refIds);
        ...
        public SelectModel GetSubStockLogsSelect(int pageIndex, int pageSize, string orderStr, string stockName, DateTime applyTimeBegin, DateTime applyTimeEnd)
        {
            ...
            sb.Clear();
            sb.AppendFormat(" sl.LogStatus={0} and sl.ContractId=0 and sl.LogDirection = {1} ", readyStatus, (int)NFMT.StockBasic.LogDirectionEnum.In);
